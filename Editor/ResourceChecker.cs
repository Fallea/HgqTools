...
public class ResourceChecker : EditorWindow
{
        ...
        switch (format)
        {
			...
			*/
            ...
            case TextureFormat.BGRA32://	 Format returned by iPhone camera
                return 32;
                //case TextureFormat.a://	 Flash-specific RGB DXT1 compressed color texture format.
    ...
    {
        if (memSizeKB < 1024) return "" + memSizeKB.ToString("0.00") + "k";
        else
        {
    ...
    void CheckResources()
    {
            ...
            foreach (Object obj in EditorUtility.CollectDependencies(new UnityEngine.Object[] { tMaterial }))
            {
                if (obj is Texture)
                {
                    ...
                    if (tTextureDetails == null)
                    {
                        ...
                        tTextureDetails.memSizeKB = (float)memSize / 1024f;
                        tTextureDetails.memSizeB = memSize;


                        TextureFormat tFormat = TextureFormat.BGRA32;
                        int tMipMapCount = 1;
                        if (tTexture is Texture2D)
                        {
                        ...
                        if (tTexture is ProceduralTexture)
                        {
                            tFormat = (tTexture as ProceduralTexture).format;
                            tMipMapCount = (int)(tTexture as ProceduralTexture).mipMapBias;
                        }
        ...
        ActiveTextures.Sort(delegate (TextureDetails details1, TextureDetails details2) { return details2.memSizeB - details1.memSizeB; });
